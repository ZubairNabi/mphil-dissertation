...
\usepackage{epsfig,graphicx,parskip,setspace,tabularx,xspace,epstopdf,url,caption,subcaption} 
...
transport. For completeness, it is worth noting that TCP was designed as a
...
\label{chapter:background:section:consistency}
...
\section{Testbed}
For all our experiments we use a machine (brand?) with 48 x86\_64 AMD Opteron
processors, with each processor clocked at 2 GHz with a 512 KB cache. The system
has 64 GB of RAM, 1 TB of HDD, and 1 Gbps network connectivity. In addition, the
system is also provisioned with a 256 GB solid state drive\footnote{We use the
SSD to negate the effect of disk I/O on shuffle time.}.
...
We first analyze the performance of a sort application. Our goal is to study the
effect of data size on the shuffle and the overall job completion time.
A 20 GB text dataset was synthetically generated to ensure equal partitioning
and equal distribution of keys, i.e. task times are not biased by unequal work
distribution. Each file in the dataset is 64 GB in size, with 640 files in
total. Each file was given as input to a single map task while the number of
reduce tasks was kept constant at 40\footnote{One per CIEL worker.}. Each CIEL
worker was configured for a single task slot.

The same experiment was run for 3 different topologies.

\subsection{Star topology - Single rack}
In this setup, all 41 containers are connected to the same switch. 

\begin{figure}[h]
  \centering
    \includegraphics[width=0.8\textwidth]{sort.eps}
    \caption{Sort - Job Completion Time - Star topology}
...
\begin{figure}[h]
  \centering
    ...
    \caption{Sort - Phase-wise Job Completion Time - Star topology}
...
implementation without requiring an external driver. To evaluate this
...
network and CPU activity throughput the course of the execution.

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.8\textwidth]{kmeans_sort.eps}
    \caption{Sort and k-means Job Completion Time - Star topology}
...
\begin{figure}
        ...
        \begin{subfigure}[b]{0.33\textwidth}
                ...
                \label{fig:10GBsortres}
        \end{subfigure}
        \caption{CPU and Network usage of kmeans}
